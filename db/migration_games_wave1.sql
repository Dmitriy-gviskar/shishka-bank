-- Волна 1 развивающих игр: memory / word / odd (+ резерв волны 2)
alter table mini_games drop constraint if exists mini_games_game_check;
alter table mini_games add constraint mini_games_game_check
  check (game in (
    'multiply', 'guess', 'count', 'chess',
    'memory', 'word', 'odd',
    'number', 'compare', 'story'
  ));
...
insert into achievements(code, title, description, metric, threshold, tier, reward) values
  ('memory_1', 'Зоркий глаз',   'Собери 12 пар в «Найди пару»', 'memory_score', 12, 1, 5),
  ('memory_2', 'Память леса',   'Собери 30 пар в «Найди пару»', 'memory_score', 30, 2, 15),
  ('word_1',   'Буквоед',       'Собери 15 слов в «Собери слово»', 'word_score', 15, 1, 5),
  ('word_2',   'Читатель леса', 'Собери 40 слов в «Собери слово»', 'word_score', 40, 2, 15),
  ('odd_1',    'Следопыт',      '10 верных ответов в «Что лишнее»', 'odd_score', 10, 1, 5),
  ('odd_2',    'Знаток групп',  '30 верных ответов в «Что лишнее»', 'odd_score', 30, 2, 15)
