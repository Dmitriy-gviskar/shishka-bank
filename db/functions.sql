...
create or replace function send_message(p_from uuid, p_to uuid, p_type text, p_content text)
...
begin
  ...
  insert into messages(circle_id, from_user, to_user, type, content)
    values (c_id, p_from, p_to, p_type, p_content) returning * into m;
  perform check_achievements(p_from);   -- «Почтальон»
  return m;
end $$;
